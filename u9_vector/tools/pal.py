...
def pal_coeffs(chord):
    ...
    for c in iter(map(lambda a:map(sum, zip(map(lambda x:x*cos(a),V),map(lambda x:x*sin(a),U))),  map(lambda x:(x-5/8)*2*pi/chord,range(chord)))):
    ...
    for c in iter(map(lambda a:map(sum, zip(map(lambda x:-x*cos(a),V),map(lambda x:x*sin(a),U))),  map(lambda x:(x-3/8)*2*pi/chord,range(chord)))):
        print "%+2.4f %+2.4f %+2.4f   uvsum #(%+4d,%+4d,%+4d)" % tuple(c+map(lambda x:int(x*(2*0.714*256/3.3)*0.72),c))
